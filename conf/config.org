...
#+begin_src emacs-lisp
  (defun company-config ()
    (setq eglot-stay-out-of '(company)) ; prevent eglot from overriding company-backends
    (global-company-mode t))
#+end_src

*** Backends

Primarly use company-capf, which display lsp completions, for prog-mode. Company-dabbrev display completions from open buffers.

#+begin_src emacs-lisp
  (setq company-prog-mode-backends '((company-capf :with company-yasnippet company-dabbrev)))
#+end_src
...
#+begin_src emacs-lisp
    (use-package company
      ...
      (defun company-prog-mode-hook ()
        (setq-local company-backends company-prog-mode-backends))
      :hook
      (prog-mode . company-prog-mode-hook)
